...
function Card({ title, description, label, image, video }) {
    ...
    const [videoClass, setVideoClass] = useState('card-video card-video--hidden');
    const [cardClass, setCardClass] = useState('card flex flex-column justify-between');
    ...
    return (
        <div className={cardClass} style={image ? {backgroundImage: `url(${image})`} : {}}>            
            <h3>{title}</h3>
            ...
            {videoAdded && <video src={video} className={videoClass} ref={videoRef} loop />}
        </div>
    )
}
